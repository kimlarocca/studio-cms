...
<div class="twd_container">
  ...
  <h3 class="twd_centered twd_margin20">Unlimited Membership: $<?php echo $row_student['membershipFee']; ?></h3>
  <?php
$startDate = date("Y-m-d", strtotime("now"));
$endDate = strtotime ( '+1 month' , strtotime ( $startDate ) ) ;
$endDate = date ( 'Y-m-d' , $endDate );
echo '<p class="twd_centered">Start Date: '.date("m/d/y", strtotime($startDate)).'</p>';
echo '<p class="twd_centered">End Date: '.date("m/d/y", strtotime($endDate)).'</p>';
?>
<div style="text-align:center"><p><span style="color:#990000">Checkout with Paydunk and get <strong>$5 off all memberships</strong> (for a limited time)!</span><br />
...
mysql_select_db($database_wotg, $wotg);
$query_order = "INSERT INTO orders (amount, firstName, lastName, emailAddress, studioID, orderInfo, orderStatus, notes, paymentID, studentID) VALUES (".$orderAmount.", '".$row_student['firstName']."', '".$row_student['lastName']."','".$row_student['emailAddress']."',".$studioID.",'Membership','pending','".$orderDescription."','".$payment->id."',".$studentID.")";
