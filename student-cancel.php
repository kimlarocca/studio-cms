...
mysql_select_db($database_wotg, $wotg);
$query_reservation = sprintf("SELECT * FROM attendance,classes WHERE attendance.attendanceID = %s AND attendance.classID=classes.classID", GetSQLValueString($colname_reservation, "int"));
...
?>
  ...
  <p class="twd_centered twd_margin20"><em>Class: <?php echo $row_reservation['name']; ?></em></p>
  <p class="twd_centered twd_margin20"><em>Date: <?php echo $row_reservation['dateAdded']; ?></em></p>
  <p class="twd_centered twd_margin20"><em> Type: <?php echo $row_reservation['attendanceType']; ?></em></p>
  <p class="twd_centered twd_margin20">Please note: if you cancel, your account will be credited 1 prepaid class which you can use to reserve a spot in any of our upcoming classes. This does not apply to members, only those who have bought single sessions or prepaid packages.</p>
  <p class="twd_centered">&nbsp;</p>
  <p class="twd_centered"><a href="student-cancel2.php?type=<?php echo $row_reservation['attendanceType']; ?>&attendanceID=<?php echo $row_reservation['attendanceID']; ?>" class="button">cancel reservation</a></p>
  <?php } ?>
</div>
