...
mysql_select_db($database_wotg, $wotg);
$query_orderInfo = "SELECT * FROM orders,classes WHERE orders.classID=classes.classID AND orders.paymentID = '".$_GET['paymentId']."'";
$orderInfo = mysql_query($query_orderInfo, $wotg) or die(mysql_error());
$row_orderInfo = mysql_fetch_assoc($orderInfo);
$totalRows_orderInfo = mysql_num_rows($orderInfo);
}
...
$clientId = 'AZMp4ST-cySZYwYzoPfmC2L7Ug25MFpIydvnvzEmdIXM1ndJ1DVAvllFkstqyrifh4K3ys_lNi729mQP';
$clientSecret = 'EDJzmm3bRi3KDjMvmAF5PsDsa0N9uSOeJtMLyb7hVYcNfbN6iyZbWpXhXLcmtVbS9hBpn5Kni4mJKlDF';
...
if ($_GET['success']=='true'){
	...
	if ($row_orderInfo['orderInfo']=='Drop In'){
	//update attendance records
	$addrecords = "INSERT INTO attendance(studentID, classID, instructorID, dateAdded, attendanceType, studioID, paymentType) VALUES (".$row_orderInfo['studentID'].",".$row_orderInfo['classID'].",".$row_orderInfo['instructorID'].",'".$row_orderInfo['classDate']."','Drop In',".$row_orderInfo['studioID'].",'PayPal')";
	mysql_select_db($database_wotg, $wotg);
	mysql_query($addrecords, $wotg) or die(mysql_error());	
	
	header("Location: student-home.php?action=reserved");
	}
	if ($row_orderInfo['orderInfo']=='Pre Paid Package'){
		//update student table
		$classesLeft = (int)$row_student['classesLeft']+$row_orderInfo['classesAdded'];
		$query_prepaid = "UPDATE students SET classesLeft = ".$classesLeft." WHERE studentID=".$row_orderInfo['studentID'];
		$prepaid = mysql_query($query_prepaid, $wotg) or die(mysql_error());
		header("Location: student-home.php?action=prepaidPaid");
	}
	if ($row_orderInfo['orderInfo']=='Membership'){
		...
		$prepaid = mysql_query($query_prepaid, $wotg) or die(mysql_error());
		header("Location: student-home.php?action=membershipPaid");
	}
}
else {
//update order database
