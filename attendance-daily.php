...
mysql_select_db($database_wotg, $wotg);
$query_attendance = "SELECT attendance.attendanceID, attendance.checkedIn, instructors.instructorID,instructors.firstName AS instructorName,instructors.lastName AS instructorLastName, classes.startTime,classes.classFee, classes.prepaidFee, classes.name as className, classes.classID, students.emailAddress, students.studentID, students.waiver, students.firstName, students.lastName,attendance.studioID,attendance.dateAdded as attendanceDate,attendance.attendanceType,attendance.addedBy,attendance.dropInName,attendance.dropInEmail,attendance.paymentType,attendance.classID,attendance.instructorID FROM attendance,students,classes,instructors WHERE attendance.studioID = ".$row_currentUser['studioID']." AND date(attendance.dateAdded) = '".$currentDate."' AND attendance.studentID = students.studentID AND attendance.classID=classes.classID AND instructors.instructorID=attendance.instructorID ORDER BY classes.startTime,students.firstName";
...
<div class="twd_container">
  <h2 class="twd_centered">Daily Attendance for <?php echo date("m/d/y", strtotime($currentDate)); ?></h2>
  ...
  <table border="0" cellpadding="5" cellspacing="0" align="center" class="twd_margin20">
	...
	?>
      <tr>
        ...
        <td><?php echo $row_attendance['instructorName']; ?> <?php echo $row_attendance['instructorLastName']; ?></td>
      ...
      <?php } ?></td>
		...
		?>
        <a href="mailto:<?php echo $row_attendance['emailAddress']; ?>?subject=please%20sign%20our%20waiver&body=Save time before class and sign our waiver now with your touch screen device: https://my.slateit.com/waiver.php?studentID=<?php echo $row_attendance['studentID']; ?>" style="color:red">email waiver link</a>
        <?php
	}
	?></td>
      </tr>
  ...
  <p class="twd_centered twd_margin20"><strong>Total # Students: <?php echo $totalRows_attendance; ?></strong></p>
  <?php } ?>
</div>
