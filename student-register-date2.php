...
mysql_select_db($database_wotg, $wotg);
$query_upcoming = "SELECT attendance.classID,attendance.studentID,attendance.dateAdded FROM attendance,classes WHERE attendance.classID=".$_GET['classID']." AND attendance.studentID = ".$row_student['studentID']." AND attendance.dateAdded = '".$today."'";
...
<div class="twd_container">
 ...
 $formattedDate = date("l, m/d/Y", strtotime($_GET['datePicked']));
  ...
  <h2 class="twd_centered twd_margin20"><?php echo $row_classes['name']; ?> on <?php echo $formattedDate; ?></h2>
  <h3 class="twd_centered twd_margin10">Cancellation Policy</h3>
  <p class="twd_centered twd_margin20">
    <?php if ($totalRows_upcoming>0) { ?>
    <span style="color:red;">You have already registered for this class! If you want to pay for a friend you can reserve multiple spots under your name, but please be advised that <u>you are liable for the class fee if your friend does not show up for class</u>.</span>
    <?php } ?>
    You may cancel your reservations up to 24 hours in advance.  Reservations are non-refundable within 24 hours of the class start time. By registering for this class you are agreeing to these terms. Please contact <?php echo $row_student['studioName']; ?> for more information about the cancellation policy. Thank you!</p>
  <?php
...
$addrecords = "INSERT INTO attendance(studentID, classID, instructorID, dateAdded, attendanceType, studioID) VALUES (".$row_student['studentID'].",".$row_classes['classID'].",".$row_classes['instructorID'].",'".$_GET['datePicked']."','Drop In',".$row_student['studioID'].")";
mysql_select_db($database_wotg, $wotg);
...
mysql_select_db($database_wotg, $wotg);
$query_order = "INSERT INTO orders (amount, firstName, lastName, emailAddress, studioID, orderInfo, orderStatus, notes, paymentID, classDate, classID, studentID) VALUES (".$orderAmount.", '".$firstName."', '".$lastName."','".$row_student['emailAddress']."',".$studioID.",'Drop In','pending','".$orderDescription."','".$payment->id."','".$dateAdded."',".$classID.",".$studentID.")";
$order = mysql_query($query_order, $wotg) or die(mysql_error());
$id = mysql_insert_id();

//return $payment;
?><!-- prepaid -->
  <h3 class="twd_centered twd_margin20">use your prepaid package</h3>
  <?php 
  ...
  <!-- dropins -->
  <h3 class="twd_centered twd_margin20" style="padding-top:20px;">single session: <?php echo money_format('%.2n', $row_classes['classFee']); ?></h3>
  <div class="twd_centered twd_margin20"><a href="<?php echo $approvalUrl; ?>"><img  class="twd_centered" src="../images/paypal.png" width="142" height="27"></a></div>
  <p class="twd_centered twd_margin20"><strong>Don't have a PayPal account?</strong> You don't need a PayPal account in order to pay with PayPal! Once you click on the PayPal button above, you will see a grey button under the login area that says “pay with debit or credit card”. Click on that if you do not have a PayPal account and you want to pay with your debit or credit card.
  <p>
