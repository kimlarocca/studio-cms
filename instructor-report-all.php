...
$colname_currentInstructor = "-1";
if (isset($_GET['instructorID'])) {
  $colname_currentInstructor = $_GET['instructorID'];
}
mysql_select_db($database_wotg, $wotg);
$query_currentInstructor = sprintf("SELECT * FROM instructors WHERE instructorID = %s", GetSQLValueString($colname_currentInstructor, "int"));
$currentInstructor = mysql_query($query_currentInstructor, $wotg) or die(mysql_error());
$row_currentInstructor = mysql_fetch_assoc($currentInstructor);
$totalRows_currentInstructor = mysql_num_rows($currentInstructor);

$colname_attendance = "-1";
if (isset($_GET['instructorID'])) {
  $colname_attendance = $_GET['instructorID'];
}
mysql_select_db($database_wotg, $wotg);
$query_attendance = sprintf("SELECT instructors.instructorID, instructors.firstName AS instructorName, instructors.lastName AS instructorName2, attendance.attendanceID,attendance.studentID,attendance.classID,attendance.instructorID,attendance.dateAdded,attendance.attendanceType,attendance.addedBy,attendance.dropInName,attendance.dropInEmail,attendance.paymentType,students.studentID,classes.classID,classes.name,classes.startTime,classes.classFee,classes.prepaidFee,students.firstName,students.lastName FROM attendance,classes,students,instructors WHERE attendance.studentID=students.studentID AND classes.instructorID=instructors.instructorID AND attendance.classID = classes.classID AND DATE(attendance.dateAdded) BETWEEN '".$_GET['startDate']."' AND '".$_GET['endDate']."' ORDER BY attendance.instructorID, attendance.dateAdded, attendance.classID");
...
<h1 class="twd_centered"><?php echo $row_currentUser['studioName']; ?> | Instructor Attendance Report</h1>

<?php include("navigation.php"); ?>
<div class="twd_container">
  <h2 style="padding-bottom:20px" class="twd_centered">Generate New Instructor Report</h2>
  ...
  <tr>
    <td>Change Instructor:</td>
    <td><label for="instructorID"></label>
      <select name="instructorID" id="instructorID">
        <?php
do {  
?>
        <option value="<?php echo $row_instructors['instructorID']?>"<?php if (!(strcmp($row_instructors['instructorID'], $_GET['instructorID']))) {echo "selected=\"selected\"";} ?>><?php echo $row_instructors['lastName']?>, <?php echo $row_instructors['firstName']?></option>
        <?php
...
?>
  ...
  <tr>
  <td>Date Range:</td>
  <td><strong><?php echo date("m/d/Y", strtotime($_GET['startDate'])); ?></strong> to <strong><?php echo date("m/d/Y", strtotime($_GET['endDate'])); ?></strong></td>
    <td>&nbsp;</td>
  </tr>
</table><input name="endDate" type="hidden" value="<?php echo $_GET['endDate']; ?>" /><input name="startDate" type="hidden" value="<?php echo $_GET['startDate']; ?>" />
      </form>
      </div>
  <h2 style="padding-bottom:20px" class="twd_centered">Report Results</h2>
  
  <?php 
  	//create array with instructor IDs
	//KIMMMMMMMMMMM
  	$i=0;
	$prevID = $row_attendance['instructorID'];
	while ($row = mysql_fetch_assoc($attendance)){
		$currentID = $row['instructorID'];
		if($prevID==$currentID) {
			$array[$i] = $currentID;
		}
		$i++;
  ...
  //do {
   ?>
  <h3 style="padding-bottom:20px" class="twd_centered"><?php echo $row_attendance['instructorName']; ?> <?php echo $row_attendance['instructorName2']; ?></h3>
  ...
  <table border="0" cellpadding="10" cellspacing="0" align="center">
	...
	?>
	  ...
	  //calculate totals
	  $averageStudents = 0;
	  if($totalClasses!=0) $averageStudents = $totalRows_attendance/$totalClasses;
	  $hourlyFee = 35;
  ...
  <h2 style="padding:20px 0 20px 0" class="twd_centered">Summary</h2>
  <table width="0" border="0" cellspacing="0" cellpadding="10" align="center">
  <tr>
    <td>Number of Students Taught:</td>
    <td><?php echo $totalRows_attendance-$totalNoShows ?></td>
  </tr>
  <tr>
...
<?php
 // } while ($row_attendance['instructorID'] != $currentInstructorID); 
?>
...
mysql_free_result($currentInstructor);
